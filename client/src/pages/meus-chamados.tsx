  ...
  return (
    <AppLayout>
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-bold text-foreground mb-1" data-testid="text-page-title">Meus Chamados</h1>
        ...
        <Link href="/abrir-chamado">
          <Button className="gap-2 rounded-lg" data-testid="button-open-ticket">
        ...
        {isLoading ? (
          ...
          <Card className="border-dashed border-2 border-border">
            <CardContent className="py-16 flex flex-col items-center gap-4 text-center">
              ...
              <Link href="/abrir-chamado">
                <Button variant="outline" className="gap-2 rounded-lg" data-testid="button-open-first-ticket">
                  <MessageSquarePlus className="w-4 h-4" />
          ...
          tickets.map(ticket => (
            <Link key={ticket.id} href={`/chamado/${ticket.id}`} data-testid={`link-ticket-${ticket.id}`}>
              <Card className="border-border hover:shadow-md cursor-pointer transition-all">
