  ...
  return (
    <AppLayout>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-4xl font-display font-bold text-foreground mb-2" data-testid="text-page-title">Service Tickets</h1>
          <p className="text-muted-foreground text-lg">Manage and resolve incoming requests.</p>
        </div>
                ...
                <Card className="transition-all duration-200 hover-elevate border-border/50 cursor-pointer">
                  <CardContent className="p-5 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {ticket.confidenceScore && (
                          <span className="text-xs flex items-center gap-1 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded-full">
                            <Sparkles className="w-3 h-3" /> {Math.round(ticket.confidenceScore * 100)}% Match
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      <div className="text-sm font-medium" data-testid={`text-ticket-date-${ticket.id}`}>{ticket.createdAt ? format(new Date(ticket.createdAt), 'MMM d, yyyy HH:mm') : 'Unknown'}</div>
                    </div>
                  </CardContent>
                </Card>
              </Link>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[600px] border-secondary/20">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl text-foreground" data-testid="text-dialog-title">Create Support Ticket</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-6 pt-4">
          <div className="space-y-2">
            <Label htmlFor="title" className="font-semibold">Title</Label>
            <Input id="title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Brief summary of the issue" required data-testid="input-ticket-title" />
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center gap-2">
              ...
              <Button 
                ...
                disabled={classifyMutation.isPending || !description}
            ...
            <div className="space-y-2">
              ...
              <Select value={category} onValueChange={setCategory}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Access">Access & Identity</SelectItem>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} data-testid="button-cancel-ticket">Cancel</Button>
